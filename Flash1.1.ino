...
// Генерация случайного символа (буквы и цифры)
char randomChar() {
...
void createFakeAP(const String &ssid, int channel, const String &password) {
  WiFi.softAPdisconnect(true);  // отключаем предыдущую AP, если была
  WiFi.softAP(ssid.c_str(), password.c_str());  // WPA2
  wifi_set_channel(channel);
  Serial.println("Создана WPA2-точка доступа: SSID = " + ssid + ", пароль = " + password);
}
...
void handleCloneAP() {
  if (server.hasArg("ssid") && server.hasArg("bssid") && server.hasArg("channel")) {
    ...
    String password = generateRandomPassword();  // случайный пароль
    createFakeAP(ssid, channel, password);

    String response = "Клонированная WPA2-точка доступа создана:\n";
    response += "SSID: " + ssid + "\n";
    response += "Канал: " + String(channel) + "\n";
    response += "Пароль: " + password + "\n";
